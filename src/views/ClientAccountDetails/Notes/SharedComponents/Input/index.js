...
import {StyledInput, Styleddatepicker, StyledButton, TextInput, StyledTextArea} from './style';
  ...
  return (
      <StyledInput>
         ...
         {props.button?<StyledButton customcornertype='round-corner' type={props.type} onClick={props.onClik}>{props.content}</StyledButton>:null}
          ...
          {props.text? <TextInput placeholder={props.placeholder} onChange={props.onChange}/>:null}
          {props.textArea?<StyledTextArea placeholder={props.placeholder} rows={5} onChange={props.onChange}/>:null}
      </StyledInput>
  )
}
